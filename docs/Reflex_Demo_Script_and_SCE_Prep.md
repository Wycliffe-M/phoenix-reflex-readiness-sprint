...
- Browser open, three tabs/windows ready: Retailer view, Dispatcher view, Rider view — all pointed at https://phoenix-reflex-readiness-sprint.onrender.com
...
**Backup plan if something breaks live:** [name which member has a phone-recorded backup video of a clean run]. One known fragile point worth having top of mind: the free-tier spin-down/reset behavior — if the app is slow to respond, or shows an empty database because it redeployed recently, that's a known, expected limitation, not a surprise failure. Say so plainly if it happens rather than looking flustered. **Action item:** this limitation isn't yet its own line in the trade-off log — worth adding a real Trade-off 6 for it (ephemeral storage / no persistent disk on free tier) before submission, so this matches what's actually documented.
...
| Demo | Rotates — whoever isn't driving the keyboard (suggested) | (rotates) |
...
*(This is a starting proposal based on existing file ownership — confirm/reassign as a group during Day 2's handoff rehearsal, not unilaterally. Note: this table should end up matching the slide-ownership table in `Reflex_Deck_Outline.md` — right now that one is still blank; reconcile both at the same time.)*